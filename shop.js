/**
 * ECODOS Shop Integration
 * משיכת והצגת מוצרים מ-ECODOS Backend
 */

const SHOP_API = 'https://ecodos-backend-production.up.railway.app/api/products';
...
function displayProducts(productsToDisplay = products) {
    ...
    if (!productsToDisplay || productsToDisplay.length === 0) {
        container.innerHTML = '<p class="no-products">המוצרים יהיו זמינים בקרוב</p>';
        return;
    }
...
function updateCart() {
    ...
    localStorage.setItem('ecodos_cart', JSON.stringify(cart));
 ...
 */
function loadCart() {
    const savedCart = localStorage.getItem('ecodos_cart');
    if (savedCart) {
